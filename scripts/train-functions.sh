...
interpolate_kn () {
	...
	(set -x; "${PROJECT_SCRIPT_DIR}/interpolate-lm.py" "${args[@]}")
}
...
train_kn_single () {
	if [ -n "${1}" ]
	then
		declare -a train_fil_names=("${!1}")
	else
		declare -a train_fil_names=("${TRAIN_FILES[@]}")
	fi
	local model_file="${2}"
	local vocab_file="${3}"

	echo "${model_file} :: ${train_fil_names[@]}"
	declare -a discounting=()
	[ -n "${CLASSES}" ] || discounting=(-kndiscount1 -kndiscount2 -kndiscount3 -kndiscount4 -kndiscount5 -kndiscount6)
	concatenate_corpora train_fil_names[@] |
...
train_kn_ip () {
	...
	local model_file="${EXPT_WORK_DIR}/kn-ip.arpa.gz"
	local vocab_file  # Word or class vocabulary, depending on model type.
	if [ -n "${CLASSES}" ]
	then
		local vocab_file="${EXPT_WORK_DIR}/class.vocab"
