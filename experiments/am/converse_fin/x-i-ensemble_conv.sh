...
nnet3_affix_src=
nnet3_affix_tgt=_ensemble_conv
xvec_suffix=_hires_conv

nnet3_affix_ivec=_offline
ivec_suffix=_hires
...
if [ $stage -le 1 ]; then
  ...
  # i-vec
  $train_cmd exp/nnet3${nnet3_affix_ivec}/ivectors_${train_set}_sp${ivec_suffix}/log/lda$lda_dim.log \
    ivector-compute-lda --total-covariance-factor=0.0 --dim=$lda_dim \
    "ark:ivector-subtract-global-mean scp:exp/nnet3${nnet3_affix_ivec}/ivectors_${train_set}_sp${ivec_suffix}/ivector.scp ark:- |" \
    ark:data/${train_set}_sp/utt2spk exp/nnet3${nnet3_affix_ivec}/ivectors_${train_set}_sp${ivec_suffix}/lda$lda_dim.mat || exit 1;
fi
...
if [ $stage -le 2 ]; then
  for data in ${train_set}_sp ${test_sets} ; do
    ...
    ivecdir=exp/nnet3${nnet3_affix_ivec}/ivectors_${data}${ivec_suffix}
    ...
    local/dump_with_xvec_ivec_ensemble.sh --cmd "$train_cmd" \
      ...
      ${xvecdir_src} \
      exp/nnet3${nnet3_affix_ivec}/ivectors_${train_set}_sp${ivec_suffix}/mean.vec \
      exp/nnet3${nnet3_affix_ivec}/ivectors_${train_set}_sp${ivec_suffix}/lda$lda_dim.mat \
      ${ivecdir} \
